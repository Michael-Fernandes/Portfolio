...
$(document).ready(function () {
  ...
  };svg = d3.select(".plot").append("svg").attr("class", "box").attr("width", width + margin.left + margin.right).attr("height", height + margin.bottom + margin.top).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

  //Make generic scale
  var axisScale = d3.scaleLinear().domain([0, 100]).range([0, 500 - margin.left - margin.right]);

  // Add the x Axis
  var axis = d3.axisBottom(axisScale);

  //Add axis to canvas
  svg.append("g").attr("transform", "translate(0," + height + ")").call(axis).attr("class", "x axis");
  ...
  function makeTitle(title) {
    ...
    svg.append("text").attr("x", width / 2).attr("y", 75).attr("text-anchor", "middle").classed("title", true).style("font-size", "16px").text(title);
  ...
  function buildDots(dots, AS) {
    ...
    svg.select(".x").transition(t).call(axis);

    var circles = svg.selectAll("circle").data(dots).enter().append("circle").classed("dots", true);

    var circleAttr = circles.attr("cx", function (d) {
      return AS(d[0]);
    }).attr("cy", function (d) {
      return height - (radius + 1) - d[1] * (radius + spaceBetweenDots) * 2;
    }).attr("r", radius).attr("opacity", 0.85).style("stroke", "none").style("fill", "white");

    circleAttr.transition(t).style("fill", colors[graphType]);
  }

  function clear() {
    d3.selectAll("circle").attr("r", 0).remove();
  }
